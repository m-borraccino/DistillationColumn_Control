...
%F: approssimazione di ordine 3 delle risponste in frequenza delle
...
%F: frd calcola la f.d.t. nelle frequenze date

%

%F: calcolo delle risposte in frequenza dell'incertezza relativa al variare
...
for k = 0.8:0.05:1.2
    for tau = 0:0.1:1.0
        pert=zeros(200,1);   %F: aggiunto per + veloce implementazione
        for i = 1:200
            om = omega(i);
            pert(i) = sqrt((k*cos(om*tau)-1)^2 + (k*sin(om*tau))^2);
        end
        pert_frd = frd(pert,omega);
        bodemag(pert_frd,'c--'), %F: stupido, ogni iterazione plotta sempre 
        grid on
        hold on
    end
    end
